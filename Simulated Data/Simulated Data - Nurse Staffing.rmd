...
```
```{r days_stats, echo=FALSE}
# Print some stats about the dates data
print(sprintf("Mean number of nurses: %.2f",
        mean(eachDay$numNurses)))
print(sprintf("Mean number of nurses on weekdays: %.2f",
        mean(eachDay$numNurses[eachDay$typeOfDay == 'weekday'])))
print(sprintf("Mean number of nurses on weekends: %.2f",
        mean(eachDay$numNurses[eachDay$typeOfDay == 'weekend'])))
```
...
```
```{r qual_of_care_hist, echo=FALSE, fig.width=3, fig.height=3}
for (w in c('weekday', 'weekend')) {
...
```{r relate_qual_of_care, echo=F, eval=F}
...
```
```{r employee_id_hist, echo=F, eval=F}
employeeHist <- function(dataSet) {
...
Then we run the function to produce the results.

```{r run_generate_data, echo=F, eval=F, results='asis'}
